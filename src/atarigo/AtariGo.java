...
public class AtariGo implements Go,Serializable {
	...
	AtariGo(Player player1, Player player2, Draw drawmethod, int dimension){
		playerOne=player1;
		playerTwo=player2;
	...
	public int nextMove() {
		if(isPlayer1Turn)
			return validate(playerOne.generatemove(size));
		else
			return validate(playerTwo.generatemove(size));
	}
	/*
	 * Deseneaza text starea jocului
	 */
	public void draw() {
		design.ilustrate();
	}
